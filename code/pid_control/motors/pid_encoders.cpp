 ...
 * @author elijah
 * @brief
 * This is the code script for controlling two GMR sensor equipped motors with
 * pid closed loop. The target rpm can be controlled by a variable resistor.
 * @version 0.1
 * @date 2023-06-30
 *
 ...
 * @param rpm_max the standard rpm of the motor. can tune this parameter to
 * change the max rpm, effectively
 *
 ...
 *
 * pin parameters are:
 * @param pin_ena_upper, pin_in1, pin_in2, pin_in3_lower, pin_in4_lower,
 * pin_enb_lower the pin number for motor control, connected to the l298n board
...
const int pin_ena_upper = 9; // needs to support pwm
const int pin_in1_upper = 8;
const int pin_in2_upper = 7;
const int pin_in3_lower = 6;
const int pin_in4_lower = 5;
const int pin_enb_lower = 4; // needs to support pwm
...
void setup() {
    ...
    digitalWrite(pin_in1_upper, HIGH);
    digitalWrite(pin_in2_upper, LOW);
    digitalWrite(pin_in3_lower, LOW);
    digitalWrite(pin_in4_lower, HIGH);
...
void control() {
    ...
    if (rpm_target < 0) {
        digitalWrite(pin_in1_upper, LOW);
        digitalWrite(pin_in2_upper, HIGH);
        digitalWrite(pin_in3_lower, HIGH);
        digitalWrite(pin_in4_lower, LOW);
    ...
    set_pwm(pin_ena_upper, pwm_cur1, pin_in1_upper, pin_in2_upper);
    set_pwm(pin_enb_lower, pwm_cur2, pin_in3_lower, pin_in4_lower);
...
void set_pwm(int pin, int pwm, int in1, int in2) { analogWrite(pin, pwm); }

 ...
 *
 * @details
 * The machine exposes two buttons, start-stop button and a self cleaning
 ...
 * When the user puts the fruit in the holding bowl to peel and presses the
 * start button, the machine goes into RUNNING state.
 *
 * =============================  STATE: RUNNING ===============================
 * Then the machine will go over several stages:
 * 1. (STATE: CLOSE) The upper and lower clamping blades (name needs to be
 * changed) will close to hold the fruit in place, when the pressure sensor
 * detects the fruit (SIGNAL: PRESSURE_UPPER_THRESHOLD_EXCEEDED), the machine
 * will go to next stage.
 * 2. (STATE: UP) The upper and lower clamping blades will work in coordination
 * to lift up the fruit and rotate it to peel the fruit. The peeling blade is
 * placed on the inner surface of the machine, and the fruit is rotated against
 * the blade to peel. When the lower clamp blade reaches the height of the
 * peeling blade (SIGNAL: UP_LIMIT_EXCEEDED), the machine will go to next stage.
 * TODO: We have not determined how to put the peeling blade at the same height
 * as the top of the fruit in the beginning of this stage.
 * TODO: We have not determined how to detect when the lower clamp blade reaches
 * the height of the peeling blade in the end.
 *
 * ==============================  STATE: EXITING ==============================
 * If the above stage finishes, the machine
 * will go to the EXITING stage
 * 3. (STATE: DOWN) The upper and lower clamping blades will work in
 ...
 * surge in pressure (SIGNAL: PRESSURE_LOWER_THRESHOLD_EXCEEDED), it is when the
 * fruit reached the holding bowl.
 * TODO: We have not determined how to avoid the peeling blade when lowering the
 * fruit.
 * 4. (STATE: OPEN) The upper and lower clamping blades will open to release the
 ...
 * machine will jump to stage 4. NOTE: If the start-stop button is pressed
 * during stage 2 (STATE: UP), the machine will jump to stage 3.
 *
 * =============================  STATE: STOPPED =============================
 * If the above stage finishes, the machine
 * will go to the STATE: STOPPED stage
 *
 * =============================  STATE: CLEANING =============================
 ...
 * 1. (STATE: CLOSE) The upper and lower clamping blades will close to cover the
 * side breach of the peeling blade.
 ...
 * 3. (STATE: OPEN)Te upper and lower clamping blades will open to release the
 * fruit.
 *
 * @version 0.1
 * @date 2023-07-01
...
// this is the desired pressure in the UP and DOWN stage, the upper motor will
// adjust its speed to reach this pressure, by finely controlling the speed of
// the upper motor using pid
const int pressure_target = 900;

// this is the pressure when we assume the fruit is grasped in the clamping
// blades in CLOSE stage
const int pressure_threshold = 980;

// this is the pressure when we assume the fruit touches the holding bowl in the
// clamping blades in DOWN stage
